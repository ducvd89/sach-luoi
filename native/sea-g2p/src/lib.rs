...
// Cổng C dùng cho Android/iOS — không kéo theo Python.
#[cfg(feature = "ffi")]
pub mod ffi;
...
mod python_module {
    ...
    impl G2P {
        ...
        fn new(dict_path: &str) -> PyResult<Self> {
            let engine = crate::g2p::G2PEngine::new(dict_path)
    ...
    fn sea_g2p_rs(m: &Bound<'_, PyModule>) -> PyResult<()> {
        m.add_class::<G2P>()?;
        m.add_class::<crate::vi_normalizer::Normalizer>()?;
